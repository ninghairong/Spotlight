...
Notification::Notification(QObject* parent) : QObject(parent)
{
   /* QWidget* pWidget = qobject_cast<QWidget*>(parent);
    ...
    m_size = QGuiApplication::primaryScreen()->availableSize();
   // m_size = pWidget->size();
...
void Notification::Push(QString content,NotifyType type, NotifyPosition pos, int nLive)
{
    std::lock_guard<std::mutex> lck(m_vecMtx);
    NotificationItem* pItem = new NotificationItem(qobject_cast<QWidget*>(parent()),
        content,
    ...
    m_bAppearEnd(false)
{
    ...
    setStyleSheet(QStringLiteral("QWidget#notification_item{border:none;border-radius:8px;background-color:white;}"
        ...
        "QPushButton#btn_close{border:none;background-color:white;background-position:center;border-image:url(:/Spotlight/res/notification_close.png);}"
        "QPushButton:hover#btn_close{border-image:url(:/Spotlight/res/notification_close_hover.png);}"));
    ...
    if (type != NotifyType::Notify_Type_None)
    {
        ...
        {
            pIcon->setPixmap(QPixmap(":/Spotlight/res/type_success.png"));
        }
        else if (type == NotifyType::Notify_Type_Error)
        {
            pIcon->setPixmap(QPixmap(":/Spotlight/res/type_error.png"));
        }
        else if (type == NotifyType::Notify_Type_Warning)
        {
            pIcon->setPixmap(QPixmap(":/Spotlight/res/type_warning.png"));
        }
        else
        {
            pIcon->setPixmap(QPixmap(":/Spotlight/res/type_information.png"));
        }
        pIcon->adjustSize();
    ...
    m_bAppearEnd(false)
{
  ...
  /*  QLabel* pTitle = new QLabel(title, this);
    ...
    if (type != NotifyType::Notify_Type_None)
    {
        ...
        setFixedSize(nFixedWidth + nLeftPadding + nRightPadding + pIcon->width() + 10,
            pContent->height()/* + pTitle->height()*/ + 2 * nTopPadding + 5);
        pIcon->move(nLeftPadding, (height() - pIcon->height()) / 2);
        pContent->move(pIcon->x() + pIcon->width() + 10, (height() - pIcon->height()) / 2);
       
    }
    else
    {
        setFixedSize(pContent->width() + nLeftPadding + nRightPadding,
            pContent->height() + nTopPadding);
        pContent->move(nRightPadding, (height() - pContent->height()) / 2);
    }
    pClose->move(width() - pClose->width() / 2 - nRightPadding, (height() - pClose->height()) / 2);
